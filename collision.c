...
int Collision_CheckTriangle(CollisionPacket *colPackage, const D3DXVECTOR3 *p1,
  ...
  if (Plane_IsFrontFacingTo(trianglePlane, &colPackage->normalizedVelocity)) {
    ...
    if (!embeddedInPlane) {
      D3DXVECTOR3 sub =
          Vec3_Sub(&colPackage->basePoint, &trianglePlane->normal);
    ...
    if (foundCollison == 0) {
      ...
      b = edgeSquaredLength * (2 * Vec3_Dot(&velocity, &baseToVertex)) -
          2.0 * edgeDotVelocity * edgeDotBaseToVertex;
      c = edgeSquaredLength * (1 - Vec3_LenSq(&baseToVertex)) +
      ...
      if (Collision_GetLowestRoot(a, b, c, t, &newT)) {
        ...
        float f =
            (edgeDotVelocity * newT - edgeDotBaseToVertex) / edgeSquaredLength;
        if (f >= 0.0 && f <= 1.0) {
...
D3DXVECTOR3 Collision_CollideAndSlide(CollisionPacket *colPackage,
  ...
  D3DXVECTOR3 eSpacePosition =
      Vec3_Div(&colPackage->R3Position, &colPackage->eRadius);
  D3DXVECTOR3 eSpaceVelocity =
      Vec3_Div(&colPackage->R3Velocity, &colPackage->eRadius);

  // Iterate until we have our final position.
  D3DXVECTOR3 finalPosition = Collision_CollideWithWorld(
      colPackage, &eSpacePosition, &eSpaceVelocity, 0);
  ...
  finalPosition = Collision_CollideWithWorld(colPackage, &finalPosition,
                                             &eSpaceVelocity, 0);
...
D3DXVECTOR3 Collision_CollideWithWorld(CollisionPacket *colPackage,
                                       const D3DXVECTOR3 *pos,
                                       const D3DXVECTOR3 *vel,
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < actors[actorIndexes[i]]->vertexCount; j += 3) {
      const Vertex a = AddVertex(actors[actorIndexes[i]]->vertices[j],
                                 actors[actorIndexes[i]]->position);
      const Vertex b = AddVertex(actors[actorIndexes[i]]->vertices[j + 1],
                                 actors[actorIndexes[i]]->position);
      const Vertex c = AddVertex(actors[actorIndexes[i]]->vertices[j + 2],
                                 actors[actorIndexes[i]]->position);
  ...
  return Collision_CollideWithWorld(
      colPackage, &newBasePoint, &newVelocityVector, ++collisionRecursionDepth);
}
